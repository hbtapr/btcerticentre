...
class CertificatesController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('frontend.certificates.create');
    }
    ...
    public function store(StoreCertificateRequest $request)
    {
        ...
        if ($request->input('cert_pdf', false)) {
            $certificate->addMedia(storage_path('tmp/uploads/' . basename($request->input('cert_pdf'))))->toMediaCollection('cert_pdf');
        }
    ...
    public function edit(Certificate $certificate)
    {
        ...
        return view('frontend.certificates.edit', compact('certificate'));
    }
    ...
    public function update(UpdateCertificateRequest $request, Certificate $certificate)
    {
        ...
        if ($request->input('cert_pdf', false)) {
            if (!$certificate->cert_pdf || $request->input('cert_pdf') !== $certificate->cert_pdf->file_name) {
                if ($certificate->cert_pdf) {
                    $certificate->cert_pdf->delete();
                }

                $certificate->addMedia(storage_path('tmp/uploads/' . basename($request->input('cert_pdf'))))->toMediaCollection('cert_pdf');
            }
        } elseif ($certificate->cert_pdf) {
            $certificate->cert_pdf->delete();
        }
