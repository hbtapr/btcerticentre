                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.questions.update", [$question->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('question') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="question" id="question" value="{{ old('question', $question->question) }}">
                        ...
                        <div class="form-group {{ $errors->has('questiontype') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="questiontype" id="questiontype" value="{{ old('questiontype', $question->questiontype) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('themes') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="themes[]" id="themes" multiple>
                                @foreach($themes as $id => $theme)
                                    <option value="{{ $id }}" {{ (in_array($id, old('themes', [])) || $question->themes->contains($id)) ? 'selected' : '' }}>{{ $theme }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('difficulty') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="difficulty" id="difficulty" value="{{ old('difficulty', $question->difficulty) }}" required>
